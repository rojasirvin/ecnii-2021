...
knitr::opts_chunk$set(echo = FALSE, warning = F, message = F, fig.path = "figures/")
...
.title[
# Clase 16. Cuestiones prácticas de VI
]
.subtitle[
## Econometría II
]
...
Si estamos dispuestos a asumir Con errores homocedásticos
...
$$\begin{align}\hat{V}(\hat{\beta}_{VI})&=N(X'Z(Z'Z)^{-1}Z'X)^{-1}(X'Z(Z'Z)^{-1}\hat{S}(Z'Z)^{-1}Z'X)(X'Z(Z'Z)^{-1}Z'X)^{-1}\end{align}$$
...
- Vale la pena mostrar cómo lucen nuestros estimadores con matrices (y esto los convencerá en el caso de estimadores de casos más particulares)

- Retomemos los datos de Card

- Noten que podemos ahorrar espacio al especificar la segunda parte de la fórumla como
...
.pull-right[
```{r echo=T}
stargazer(iv_ei, type="text", keep=c("educ"))
```
]
]
...
#Matriz de varianzas (con correción por muestras finitas N/N-k)
V=sigma2*solve(t(X)%*%P%*%X)* N/(N-k)

#Comparamos el coeficiente y ee de educación con lo obtenido con ivreg
b[2,1]
sqrt(diag(V))
```
]
...
.tiny[
```{r echo=T}
V_ei_r <- vcovHC(iv_ei, type = "HC1")
se_ei_r <- sqrt(diag(V_ei_r))

#En este caso los errores son más pequeños. Con VI pueden suceder cosas raras
stargazer(iv_ei, iv_ei,
          type="text",
          se = list(NULL, se_ei_r),
          keep = c("educ"),
          digits = 4,
...
# Caso heterocedástico con matrices

.pull-left[

- Queremos estimar (ver CT p. 186)

$$\begin{align}\hat{V}(\hat{\beta})&=N(X'Z(Z'Z)^{-1}Z'X)^{-1}\times \\ &(X'Z(Z'Z)^{-1}\hat{S}(Z'Z)^{-1}Z'X)\times \\ &(X'Z(Z'Z)^{-1}Z'X)^{-1}\end{align}$$
con $\hat{S}=Z'DZ/N$, y donde $D=diag[\hat{u}_i^2]$
 
.tiny[
```{r echo=T}
#Para el caso heterocedástico
...
Haremos precisiones para la estimación de errores estándar
